...
{
	// initialise queue
	Queue levelOrder = newQueue();

	// add t's root node to queue
	if (t != NULL) {
		QueueJoin(levelOrder, t);
	}
	// while queue still has some entries
	while (QueueIsEmpty(levelOrder) == false) 
	{
		// take the head of the queue
		struct BSTNode *curr = QueueLeave(levelOrder);
		...
		if (curr->left != NULL)
		{
			QueueJoin(levelOrder, curr->left);
		}
		// add the right child (if any) to the queue
		if (curr->right != NULL)
		{
			QueueJoin(levelOrder, curr->right);
		}
	}
	return;
}
